...
namespace EnergyOptimizer.API.Services
{
    public class EnergyReadingSimulatorService : BackgroundService
    {
        ...
        private async Task GenerateReadingsAsync()
        {
            ...
            var liveReadings = new List<LiveReadingDto>();  // For SignalR updates (Broadcasting)
        ...
        private async Task BroadcastReadings(List<LiveReadingDto> liveReadings)
        {
            try
            {
                await _hubContext.Clients.All.SendAsync("ReceiveLiveReadings", liveReadings);

                var readingsByZone = liveReadings
                    .GroupBy(r => r.ZoneName)
                    .ToDictionary(g => g.Key, g => g.ToList());
                foreach (var reading in readingsByZone) { 
                
                    await _hubContext.Clients.Group(reading.Key).SendAsync("ReceiveZoneReadings", reading.Value);
                }

                _logger.LogInformation("Broadcasted live readings to clients");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error broadcasting live readings");
            }
        }
